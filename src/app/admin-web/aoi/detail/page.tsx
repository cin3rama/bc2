...
export default async function AdminWebAoiDetailPage({
    ...
    searchParams: Promise<{ aoiId?: string }>;
}) {
    const { aoiId } = await searchParams;
    const displayAoiId = aoiId?.trim() || "—";
        ...
        <main className="flex flex-col gap-4">
            <AdminSessionGate>
                <section className="grid gap-4 xl:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle>Actor Policy Editor Shell — AOI #{displayAoiId}</CardTitle>
                        </CardHeader>
                        <CardContent>
